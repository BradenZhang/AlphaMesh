...
class _UnavailableMixin:
    provider_name = ProviderName.IBKR
    transport = "client_portal"

    def _error(self) -> ValueError:
        return ValueError(
            "IBKR connector scaffold exists, but the API bridge is not "
            "implemented yet."
        )
    ...
    def healthcheck(self, capability: str) -> ConnectorHealth:
        return ConnectorHealth(
            ...
            available=False,
            message="IBKR connector scaffold exists, but the API bridge is not implemented yet.",
        )


class IbkrMarketConnector(_UnavailableMixin, MarketDataConnector):
...
class IbkrExecutionConnector(_UnavailableMixin, ExecutionConnector):
...
class IbkrAccountConnector(_UnavailableMixin, AccountConnector):
